...
# def all_products(request):
#     f = ProductFilter(request.GET, queryset=Product.objects.all())
#     return render(request, 'shop/products.html', {'filter': f, 'brands': Brand.objects.filter(favorite=True), 'categories': Category.objects.all()})
...
#     q = request.GET.get('q')
#     if q is None:
#         messages.error(request, 'Pleace enter somthing to search')
...
# #     sellers = Seller.objects.get(slug=sellers)
# #     products = Product.objects.filter(slug=sellers)
# #     return render(request, 'shop/seller_list.html', {
# #         'sellers': sellers
# #         # 'products': products,
